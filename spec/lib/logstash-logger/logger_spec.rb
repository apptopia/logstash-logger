...
describe LogStashLogger do
  shared_examples "logstash logger" do
  ...
  context "when transport is redis" do
    let(:host) { '127.0.0.1' }
    ...
    it 'uses a LogStashLogger::Redis as the log device' do
      ...
      dev.should be_a LogStashLogger::Redis
      dev.instance_variable_get(:@redis_list).should == 'logstash-list'
    end

    # The raw input received by the logstash listener
    let :listener_input do
      redis.lpop 'logstash-list'
    end

    it_should_behave_like "logstash logger"
  end

  context "when transport is socket" do
